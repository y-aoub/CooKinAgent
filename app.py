...
parser.add_argument('--build_chroma', action='store_true', help='Whether to build chroma vectors using your resources or not.')
...
def streamlit_app(build_chroma, style=STYLE):
    ...
    st.markdown("<h1 class='title'>Welcome to the Wacky World of CooKinAgent!</h1>", unsafe_allow_html=True)
    ...
    with st.spinner("We're in the process of making the recipes database in the adpated format, which will be a one-time endeavor! It will take some time, but your patience will be rewarded with the best recipes! 😄"):
        build_vectorsctore(build_chroma=build_chroma)

    city = st.text_input("Enter the name of a city you're interested in!", placeholder="City Name", key="city_input")
    if st.button("Whip Up Some Recipes!", key="get_info"):
        if city:
            ...
            st.markdown("<p class='warning-text'>Hey there, sparky! Don't leave me hanging! Enter a city name and let's cook up some fun!</p>", unsafe_allow_html=True)
