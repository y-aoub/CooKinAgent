...
def build_vectorstore(build_chroma):
    ...
    if build_chroma:
        gdown_raw_data(
            recipes_data_id=RECIPES_DATA_ID,
...
def main(city):
    ...
    suggesting_system_prompt = read_txt_file(
        data_text_path=SUGGESTING_SYSTEM_PROMPT_PATH
    )
    formatting_user_prompt = read_txt_file(data_text_path=FORMATTING_USER_PROMPT_PATH)
    formatting_system_prompt = read_txt_file(
        data_text_path=FORMATTING_SYSTEM_PROMPT_PATH
    )

    suggesting_user_prompt = set_user_prompt(
        *(suggesting_user_prompt, *filtered_weather_response)
    )
    ...
    formatting_user_prompt = set_user_prompt(
        *(
            ...
            *filtered_weather_response,
            unformatted_agent_response,
        )
    )
